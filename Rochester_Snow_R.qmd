...
```{r}
stl_detrending <- stl(max_temps, s.window = "periodic", t.window = 135)
plot(stl_detrending)
```
...
```{r}
motemps.train <- window(max_temps, start = c(1970, 1), end = c(2024,12))
motempts.fore <- window(max_temps, start = c(2025,1), end = c(2025, 9))

n_ttrain <- length(motemps.train)
n_tfore <- length(motempts.fore)
